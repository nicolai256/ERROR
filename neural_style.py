...
def stylize(content_img, style_imgs, init_img, frame=None):
  with tf.device("/job:localhost/replica:0/task:0/device:GPU:0"),tf.compat.v1.Session() as sess:
    # setup network
...
def get_content_image(content_img):
  ...
  if h > w and h > mx:
    ...
    img = cv2.resize(img, dsize=(int(h), mx), interpolation=cv2.INTER_AREA)
  if w > mx:
    h = (float(mx) / float(w)) * h
    img = cv2.resize(img, dsize=(mx, int(w)), interpolation=cv2.INTER_AREA)
  img = preprocess(img)
  return img
